...
const AdministratorJurusan = ({ data }) => {
  ...
  const getAllDepartementsAPI = useCallback(async (keyword, limit, page) => {
    try {
      setLoading(true);
      const response = await getAllDepartements(keyword, limit, page);
  ...
  useEffect(() => {
    getAllDepartementsAPI(keyword, limit, currPage);
  }, [getAllDepartementsAPI, keyword, limit, currPage]);
          ...
          <div className="flex flex-col gap-4 xl:col-span-9">
            <Card className="w-full bg-white rounded-lg overflow-hidden h-fit">
              <CardBody className="p-4 md:px-6 lg:px-10 flex flex-col gap-3">
                <h3 className="font-semibold text-base md:text-xl">Cari jurusan</h3>
            ...
            <Card className="w-full bg-white rounded-lg overflow-hidden">
                  ...
                  <tbody className="text-center">
                    {loading ? (
                      ...
                      departements?.map((departement) => (
                        <tr className="even:bg-blue/5 hover:bg-blue/5" key={departement.id}>
                          <td className="py-6">{departement.name}</td>
                          <td className="py-6">{departement.code}</td>
                          <td className="py-6">
                            <Link href={`/administrator/jurusan/${departement.id}/edit`}>
              ...
              <CardFooter className="flex flex-col items-center justify-center p-6 gap-5">
                {departements?.length > 0 && <Paginate pageChange={pageChange} pages={pages} />}

                <Link href="/administrator/jurusan/tambah">
...
export default AdministratorJurusan;
