...
import { getDetailCollection, setUpdateCollection } from '../../../../services/collection';

const AdministratorEditKoleksi = ({ data }) => {
  ...
  const getDetailCollectionAPI = useCallback(async (collection_id) => {
    try {
      setLoading(true);
      const response = await getDetailCollection(collection_id);
  ...
  const { id: collection_id } = router.query;

  useEffect(() => {
    getDetailCollectionAPI(collection_id);
  }, [getDetailCollectionAPI, collection_id]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setLoading(true);
      const response = await setUpdateCollection(collection_id, data);
      ...
      toast.success('Yeay! Sukses edit koleksi.');
      setErrors({});
      router.push('/administrator/koleksi');
    } catch (error) {
    } finally {
      setLoading(false);
    }
        ...
        <main className="px-4 md:px-12 lg:px-24 mt-6 mb-20 lg:my-6 2xl:px-[15%] xl:grid xl:grid-cols-12 xl:gap-4">
          ...
          <Card className="w-full bg-white rounded-lg overflow-hidden h-fit xl:col-span-9">
            <CardHeader className="w-full border-l-4 border-grayish-blue bg-pastel-grey p-4 bg-lynch">
              <h2 className="text-white text-lg md:text-xl">Edit Koleksi</h2>
            </CardHeader>
              ...
              <div className="flex flex-col gap-1">
                ...
                >
                  <div className="relative">
                    ...
                    >
                      <span className="block truncate text-start">{collection.visibility}</span>
...
export default AdministratorEditKoleksi;
