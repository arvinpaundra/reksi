...
import { getExaminedRepositories } from '../../../../services/repository';
...
const RepositoriDiuji = (props) => {
  ...
  const getMentoredRepositoriesAPI = useCallback(
    async (
      ...
      try {
        ...
        const response = await getExaminedRepositories(
          pemustaka_id,
          keyword,
          collection_id,
          departement_id,
  ...
  return (
    <>
      <div>
        <Head>
          <title>Karya Tulis Ilmiah Yang Diuji</title>
        </Head>
                  ...
                  <CardBody>
                    {repositories?.length > 0 ? (
                      repositories?.map((item) => (
                        <Link href={`/dosen/repositori/diuji/${item.id}`} key={item.id}>
                          <a>
...
export default RepositoriDiuji;
