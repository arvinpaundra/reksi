...
import { getMentoredRepositories } from '../../../../services/repository';
...
const RepositoriDimbing = (props) => {
  ...
  const getMentoredRepositoriesAPI = useCallback(
    async (
      pemustaka_id,
      keyword,
      collection_id,
      ...
      try {
        ...
        const response = await getMentoredRepositories(
          pemustaka_id,
          keyword,
          collection_id,
          category_id,
  ...
  return (
    <>
      <div>
        <Head>
          <title>Karya Tulis Ilmiah Yang Dibimbing</title>
        </Head>

        <div className="w-full min-h-screen flex flex-col justify-between">
          <div>
            <Navbar active="mahasiswa" role="dosen" />
                  ...
                  <CardBody>
                    {repositories?.length > 0 ? (
                      repositories?.map((item) => (
                        <Link href={`/dosen/repositori/dibimbing/${item.id}`} key={item.id}>
                          <a>
...
export default RepositoriDimbing;
